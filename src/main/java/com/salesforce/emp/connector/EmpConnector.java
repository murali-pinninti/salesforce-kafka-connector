...
public class EmpConnector {
	 ...
	 *         Subscription or throws a CannotSubscribe exception
	 */
	...
	public void unsubscribe(String topic) {
		subscriptions.stream().filter(subscription -> subscription.getTopic().equalsIgnoreCase(topic)).findAny()
	...
	/**
	 * Subscribe to a topic, receiving events from the latest event position in the
	 * stream
	 *
	 * @param topic    - the topic to subscribe to
	 * @param consumer - the consumer of the events
	...
	/**
	 * Listens to /meta/connect channel messages and handles 401 errors, where
